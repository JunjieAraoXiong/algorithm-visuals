...
- Use brief, purposeful motion to show what changed between steps: keep nodes stationary, update stored links only when the code changes `.next`, place the new state immediately, and animate a thin dashed cue from the old target to the new target when a variable reference moves. For a stored `.next` assignment, briefly emphasize the edited field or its new solid link; never draw a cue from the old target node to the new target node that could imply a link between them. Route a switch from `None` to another head outside the chains so it never looks like a stored `.next` link. Keep cues short, cancel stale cues on rapid input or resize, and honor `prefers-reduced-motion` with an instant state change.
